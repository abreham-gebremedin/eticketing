...
use DNS1D;
use Milon\Barcode\BarcodeImage;
...
class ProductController extends Controller
{
    ...
    public function productWarehouseData($id)
    {
        ...
        if($lims_product_data->is_variant) {
            ...
            $lims_product_warehouse_data = Product_Warehouse::select('warehouse_id', 'is_new', 'qty')->where('product_id', $id)->orderBy('warehouse_id', 'asc')->get();
        ...
        foreach ($lims_product_warehouse_data as $key => $product_warehouse_data) {
            ...
            if($product_warehouse_data->is_new) {
                 $status ="new";
             }
            else {
                $status = 'Second hand(old)';
            }
        ...
        $product_warehouse = [$warehouse, $qty, $batch, $expired_date, $imei_number,$sts];
    ...
    public function printBarcode()
    {
        ...
        // $barcode = DNS1D::getBarcodeSVG('72782608', 'C39');
        // $base64 = base64_encode($barcode);
        // $html = "<img src='data:image/svg+xml;base64, $base64' />";
        // return $html;
    ...
    {$product_code = explode("(", $request['data']);
        ...
        if($lims_product_data->is_variant)
             $barcode =  DNS1D::getBarcodeSVG($lims_product_data->item_code, $lims_product_data->barcode_symbology);
        else
            $barcode =  DNS1D::getBarcodeSVG($lims_product_data->code, $lims_product_data->barcode_symbology);
        $base64 = base64_encode($barcode);
        $product[] = $base64 ;
    ...
    // {
    //     return DNS1D::getBarcodePNG('72782608', 'C39' );
    // }
