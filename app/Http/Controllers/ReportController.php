...
class ReportController extends Controller
{
    ...
    public function profitLoss(Request $request)
    {
        ...
        ->groupBy('product_id', 'product_batch_id')->get();
            ...
            if($warehouse_id == 0) {
                if($product->is_variant) {
                    ...
                    foreach ($variant_id_all as $key => $variant_id) {
                        ...
                        //purchase data
                        $shoppurchased_amount=ProductPurchase::where([
                            ['product_id', $product->id],
                            ['variant_id', $variant_id],
                            ['is_transfered', 0],

                    ])->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->sum('total');
                        $warehousepurchased_amount=warehouseProductPurchase::where([
                            ['product_id', $product->id],
                            ['variant_id', $variant_id],
                     ])->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->sum('total');

                        $nestedData['purchased_amount'] = $shoppurchased_amount+ $warehousepurchased_amount;
                        ...
                        $lims_warehouse_product_purchase_data = warehouseProductPurchase::select('purchase_unit_id', 'qty')->where([
                                            ['product_id', $product->id],
                                            ['variant_id', $variant_id]
                                    ])->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->get();

                        $purchased_qty = 0;
                        $warehouse_purchased_qty = 0;
                        ...
                        if(count($lims_warehouse_product_purchase_data)) {
                            foreach ($lims_warehouse_product_purchase_data as $product_purchase) {
                                $unit = DB::table('units')->find($product_purchase->purchase_unit_id);
                                if($unit->operator == '*'){
                                    $warehouse_purchased_qty += $product_purchase->qty * $unit->operation_value;
                                }
                                elseif($unit->operator == '/'){
                                    $warehouse_purchased_qty += $product_purchase->qty / $unit->operation_value;
                                }
                        ...
                        $nestedData['purchased_qty'] = $purchased_qty + $warehouse_purchased_qty;
                        //transfer data
    ...
    public function purchaseReport(Request $request)
    {
        ...
        foreach ($lims_product_all as $product) {
            ...
            $lims_warehouse_product_purchase_data = null;
            $variant_id_all = [];
            $warehousevariant_id_all = [];
             if($warehouse_id == 0) {
                if($product->is_variant){
                ...
                $warehousevariant_id_all = warehouseProductPurchase::distinct('variant_id')->where('product_id', $product->id)->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->pluck('variant_id');
                }else{
                ...
                $lims_warehouse_product_purchase_data = warehouseProductPurchase::where('product_id', $product->id)->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->first();
                }
            }else if($warehouse_id == 3) {
                if($product->is_variant)
                    $variant_id_all = DB::table('warehousepurchases')
                        ->join('warehouse_product_purchases', 'warehousepurchases.id', '=', 'warehouse_product_purchases.warehouse_purchase_id')
                        ->distinct('variant_id')
                        ->where([
                            ['warehouse_product_purchases.product_id', $product->id],
                            ['warehousepurchases.warehouse_id', $warehouse_id]
                        ])->whereDate('warehousepurchases.created_at','>=', $start_date)
                          ->whereDate('warehousepurchases.created_at','<=', $end_date)
                          ->pluck('variant_id');
                else
                    $lims_product_purchase_data = DB::table('warehousepurchases')
                        ->join('warehouse_product_purchases', 'warehousepurchases.id', '=', 'warehouse_product_purchases.warehouse_purchase_id')->where([
                                ['warehouse_product_purchases.product_id', $product->id],
                                ['warehousepurchases.warehouse_id', $warehouse_id]
                        ])->whereDate('warehousepurchases.created_at','>=', $start_date)
                          ->whereDate('warehousepurchases.created_at','<=', $end_date)
            ...
            if ($warehouse_id==0) {
                # code...
                if($lims_product_purchase_data || $lims_warehouse_product_purchase_data) {
                ...
                elseif(count($variant_id_all) ||count($warehousevariant_id_all)) {
    ...
    public function warehouseReport(Request $request)
    {
        ...
        if ($warehouse_id==3) {
            ...
            $lims_purchase_data = warehousePurchase::where('warehouse_id', $warehouse_id)->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->orderBy('created_at', 'desc')->get();
            ...
            foreach ($lims_purchase_data as $key => $purchase) {
                $lims_product_purchase_data[$key] = warehouseProductPurchase::where('warehouse_purchase_id', $purchase->id)->get();
            }
