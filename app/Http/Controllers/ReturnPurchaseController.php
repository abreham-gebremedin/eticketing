...
class ReturnPurchaseController extends Controller
{
    ...
    public function create(Request $request)
    {
        ...
        if ($role->hasPermissionTo('purchase-return-add')) {
            if (Auth::user()->role_id > 2) {
                if (Str::startsWith($request->input('reference_no'), "transfered-wpr-")) {
                    // Text to be removed found in the string
                    $lims_purchase_data = Purchase:: where('reference_no', $request->input('reference_no'))
                        ->first();
                ...
                # code...
                $lims_purchase_data = Purchase::select('id')
            ...
            if ($lims_purchase_data) {
                ...
                } elseif (Str::startsWith($request->input('reference_no'), "wpr-")) {
                    $lims_purchase_data = warehousePurchase:: where('reference_no', $request->input('reference_no'))
                        ->first();
                    if ($lims_purchase_data) {
                        ...
                        return redirect()->back()->with('not_permitted', 'Sorry! we can not find your data');
                    }
                } else {
                    $lims_purchase_data = Purchase::where('reference_no', $request->input('reference_no'))
                        ->first();
    ...
    {
        $isreturn=$request->input('is_return');
        if (!isset($isreturn)) {
            # code...
           return  redirect('return-purchase')->with("not_permitted","There is no selected purchase record for return");
        }

       if (Str::startsWith($request->input('reference_no'), "transfered-wpr-")) {
        # code...
        $data = $request->except('document');
        Arr::forget($data, 'reference_no');
        $data['user_id'] = Auth::id();
        $lims_purchase_data = Purchase::where("id", $data['purchase_id'])->first();
        $lims_purchase_data->returned_qty += $data['total_qty'];
        ...
        $warehouse_name = Warehouse::where('is_active', true)->where('id', $data['warehouse_id'])->first();
        $data['reference_no'] = 'returned-from-'.$warehouse_name->name.'-to-main-warehouse-'.$lims_purchase_data->reference_no."-on-".date("Y-m-d H:i:s");

        $lims_return_data = ReturnPurchase::create($data);

        $dataad['user_id'] = Auth::id();
        $dataad['created_at'] = $lims_return_data->created_at ;
        ...
        $product_code_selected = $data['is_return'];
        ...
        $tax = $data['tax'];
        $total = $data['subtotal'];
         foreach ($product_code_selected as $pcs) {
            $key = array_search($pcs, $data['product_code']);

            //return $key;
            if (Str::contains($pcs,'-')) {
                # code...
                $lims_product_selected_data = ProductVariant::where("item_code",$pcs)->first();
                $pro_id=$lims_product_selected_data->product_id;

            } else {
                # code...
                $lims_product_selected_data = Product::where('code',$pcs)->first();
                $pro_id=$lims_product_selected_data->id;
            }
            ...
            if ($purchase_unit[$key] != 'n/a') {
                ...
                if ($lims_product_data->is_variant) {
                    ...
                    $variant_id = $variant_data->id;
                     ...
                     $update_warehousepurchase_data = ProductPurchase::where([
                        ['product_id', $pro_id],
                        ['variant_id', $lims_product_variant_data->variant_id],
                        ['purchase_id', $data['purchase_id']]
                    ])->first();
                    $update_warehousepurchase_data->returned_qty=$update_warehousepurchase_data->returned_qty +  $quantity;
            ...
            PurchaseProductReturn::insert(
                ['purchase_return_id' => $lims_return_data->id, 'product_id' => $pro_id, 'product_batch_id' => $product_batch_id[$key], 'variant_id' => $variant_id, 'imei_number' => "", 'qty' => $qty[$key], 'purchase_unit_id' => $purchase_unit_id, 'net_unit_cost' => $net_unit_cost[$key], 'discount' => $discount[$key], 'tax_rate' => $tax_rate[$key], 'tax' => $tax[$key], 'total' => $total[$key], 'created_at' => \Carbon\Carbon::now(), 'updated_at' => \Carbon\Carbon::now()]
            );
        }
        ...
        $lims_purchase_data->returned_qty=$lims_purchase_data->returned_qty + $data['total_qty'];
        $lims_purchase_data->save();
        $data['user_id'] = Auth::id();
        $data['supplier_id'] = $lims_purchase_data->supplier_id;
        $data['warehouse_id'] = $lims_purchase_data->warehouse_id;
        ...
        $warehouse_name = Warehouse::where('is_active', true)->where('id', $data['warehouse_id'])->first();
        $data['reference_no'] = 'returned-from-'.$warehouse_name->name.'-to-Suppler-'.$lims_purchase_data->reference_no."-on-".date("Y-m-d H:i:s");
        ...
        $accountType = ChartofAccount::where('name', 'Purchase')->first();
        $transaction->chartof_accounts_id = $accountType->id;
        $transaction->warehouse_purchase_return_id = $lims_return_data->id;
        $transaction->save();
        ...
        if ($lims_purchase_data->payment_status==2) {
            # code...
            $transaction = new AccountTransaction;
            $transaction->reference_no =  $data['reference_no'];
            $transaction->date = date("Y-m-d H:i:s");
            ...
            $transaction->debit = array_sum($data['subtotal']);
            $accountType = ChartofAccount::where('name', 'Cash In Hand')->first();
            $transaction->chartof_accounts_id = $accountType->id;
            $transaction->warehouse_purchase_return_id = $lims_return_data->id;
            $transaction->save();
        ...
        if ($lims_purchase_data) {
            ...
            // handle the case where the purchase data is not found
        }
         $data['supplier_id'] = $lims_purchase_data->supplier_id;
        $data['warehouse_id'] = $lims_purchase_data->warehouse_id;
        $document = $request->document;
        ...
        $warehouse_name = Warehouse::where('is_active', true)->where('id', $data['warehouse_id'])->first();
        $data['reference_no'] = 'returned-from-'.$warehouse_name->name.'-to-main-Suppler-'.$lims_purchase_data->reference_no."-on-".date("Y-m-d H:i:s");
        $lims_return_data = ReturnPurchase::create($data);         
        ...
        $total = $data['subtotal'];
         foreach ($product_code_selected as $pcs) {
            ...
            if (Str::contains($pcs,'-')) {
                ...
                # code...
                $lims_product_selected_data = Product::where('code',$pcs);
                $pro_id=$lims_product_selected_data->id;
...
//     $data = $request->except('document');
//      if (Str::startsWith( $data['reference_no'], "transfered-wpr-"))  {
//         return redirect()->back()->with('not_permitted', 'Transfered');

//     }
    
    
//     if(Str::startsWith($data['reference_no'], "wpr")) { 
//         return redirect()->back()->with('not_permitted', 'WPR');

    
//  }else{ 

//       return redirect()->back()->with('not_permitted',"pr");
//   }
    ...
    {
        $lims_product_return_data = PurchaseProductReturn::where('purchase_return_id', $id)->get();
    ...
    {
        $data = $request->all();
        $lims_return_data = ReturnPurchase::find($data['purchase_return_id']);
        //return $lims_return_data;
        $lims_product_return_data = PurchaseProductReturn::where('purchase_return_id', $data['purchase_return_id'])->get();
    ...
    public function update(Request $request, $id)
    {
        ...
        foreach ($product_id as $key => $pro_id) {
            ...
            $product_return['purchase_return_id'] = $id;
    ...
    {
        $purchase_return_id = $request['returnIdArray'];
        foreach ($purchase_return_id as $id) {
            $lims_return_data = ReturnPurchase::find($id);
            $lims_product_return_data = PurchaseProductReturn::where('purchase_return_id', $id)->get();
