...
use App\product_sale_purchase_cost;
use App\Shareholder;
...
class SaleController extends Controller
{
    ...
    public function saleData(Request $request)
    {
        ...
        if (!empty($sales)) {
            foreach ($sales as $key => $sale) {
                ...
                if ($sale->total_qty<=$sale->transferred_qty) {
                    # code...
                    $nestedData['transfer'] ='Resold';

                } else {
                    # code...
                    $nestedData['transfer'] = '<a href="transfer-purchase/create?reference_no='.$sale->reference_no.'"'."class='btn btn-primary'>Transfer</a>";
    ...
    {            
        ...
        if ($role->hasPermissionTo('sales-add')) {
            ...
            return view('sale.create', compact('lims_account_list','lims_customer_list', 'lims_warehouse_list', 'lims_biller_list', 'lims_pos_setting_data', 'lims_tax_list', 'lims_reward_point_setting_data'));
        } else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
    ...
    public function store(Request $request)
    {
        ...
        $dataad['created_at'] = $lims_sale_data->created_at ;
        $dataad['warehouse_id'] =$lims_sale_data->warehouse_id;
        $dataad['reference_no'] = $lims_sale_data->reference_no ;
        $dataad['reason'] = $lims_sale_data->reference_no ;
        ...
        foreach ($product_id as $i => $id) {
            ...
            if ($sale_unit[$i] != 'n/a') {
                ...
                if ($data['sale_status'] == 1) {
                    ...
                    if ($lims_product_data->is_variant) {
                        ...
                        $lims_product_warehouse_data = Product_Warehouse::FindProductWithVariant($id, $lims_product_variant_data->variant_id, $data['warehouse_id'],1)->first();
                        ...
                        $lims_product_warehouse_data = Product_Warehouse::FindProductWithoutVariant($id, $data['warehouse_id'],1)->first();
                    }
            ...
            $product_sale['warehouse_id'] =$lims_sale_data->warehouse_id;
            $recent_sale = Product_Sale::create($product_sale);
            // $recent_sale = Product_Sale::latest()->first();

            $currentSale->push($recent_sale);
        ...
        if ($mail_data['email'] && $data['sale_status'] == 1) {
         ...
         if ($data['payment_status'] == 3 || $data['payment_status'] == 4 || ($data['payment_status'] == 2 && $data['pos'] && $data['paid_amount'] > 0)) {
            ...
            if ($cash_register_data)
             ...
             $lims_account_data = Account::where('id', $request->input('account_id'))->first();
            ...
            $lims_payment_data->change = $data['grand_total'] - $data['paid_amount'];
            ...
            foreach ($currentSale as $product_sale) {
                ...
                // $product_sold = DB::table('product_sales')
                ...
                //     return $query->where('product_sales.product_batch_id', $product_batch_id);
                // })
                // ->when($variant_id, function ($query) use ($variant_id) {
                //     return $query->where('product_sales.variant_id', $variant_id);
                // })
                // ->select(DB::raw('SUM(net_unit_price) as total_unit_price'), DB::raw('SUM(qty) as sold_qty'))
                // ->first();
            
            $product_purchase_data = ProductPurchase::where('product_id', $product_id)
            ->where('warehouse_id', $product_sale['warehouse_id'])
            ->where('is_new',1)
            ->when($product_batch_id, function ($query) use ($product_batch_id) {
            ...
            // $past_sold_qty = max(0, $product_sold->sold_qty - $sold_qty);
            // $flagcheck = 0;
            
            // foreach ($product_purchase_data as $product_purchase) {
            //     $past_purchased_qty = $product_purchase->qty;
            //     $unit_cost = $product_purchase->total / $product_purchase->qty;
            
            //     if ($past_purchased_qty > $past_sold_qty) {
            //         $flag = $past_purchased_qty - $past_sold_qty;
            //         $flagcheck++;
            ...
            //                 break;
            //             } else {
            //                 $purchased_amount = $unit_cost * $flag;
            //                 $sold_qty -= $flag;
            //             }
            //         } else {
            //             if ($product_purchase->qty >= $sold_qty) {
            //                 $purchased_amount += $unit_cost * $sold_qty;
            //                 break;
            //             } else {
            ...
            foreach ($product_purchase_data as $product_purchase) {
                $qtyforsale = $product_purchase->qty-($product_purchase->sold_qty+$product_purchase->returned_qty);

                if ($sold_qty>0 && $qtyforsale>0) {
                    # code...
                    if ($sold_qty > $qtyforsale) {
                        # code...
                        $sold_purchase = ProductPurchase::where('id', $product_purchase->id)->first();
                        $sold_purchase->sold_qty+=$qtyforsale;
                        $sold_purchase->is_sold=1;
                        $unit_cost = $product_purchase->net_unit_cost;
                        $purchased_amount += $unit_cost * $qtyforsale;
                        $sold_qty-=$qtyforsale;


                        $product_sale_purchase_cost= new product_sale_purchase_cost();
                        $product_sale_purchase_cost->product_purchase_id=$sold_purchase->id;
                        $product_sale_purchase_cost->product_sale_id=$product_sale->id;
                        $product_sale_purchase_cost->qty=$qtyforsale;
                        $product_sale_purchase_cost->save();
                        $sold_purchase->save();


                    }elseif($sold_qty==$qtyforsale) {
                        # code...
                        $sold_purchase = ProductPurchase::where('id', $product_purchase->id)->first();
                        $sold_purchase->sold_qty+=$qtyforsale;
                        $sold_purchase->is_sold=1;
                        ...
                        # code...
                        $sold_purchase = ProductPurchase::where('id', $product_purchase->id)->first();
                        $sold_purchase->sold_qty+=$sold_qty;
                        $sold_purchase->is_sold=0;

                        $product_sale_purchase_cost= new product_sale_purchase_cost();
                        $product_sale_purchase_cost->product_purchase_id=$sold_purchase->id;
                        $product_sale_purchase_cost->product_sale_id=$product_sale->id;
                        $product_sale_purchase_cost->qty=$sold_qty;
                        $product_sale_purchase_cost->save();
                        $sold_purchase->save();
            ...
            // $current_product_sale->net_unit_cost = $purchased_amount;
            ...
            if ($data['sale_status'] == 1) {
                ...
                if ($data['payment_status'] == 3 || $data['payment_status'] == 4 || ($data['payment_status'] == 2 && $data['pos'] && $data['paid_amount'] > 0)) {
                    ...
                    $transaction->reference_no = $lims_sale_data->reference_no;
                    $transaction->date = date("Y-m-d H:i:s");
                    $transaction->user_id = Auth::id();
                    $transaction->debit = $product_cost;
                    $transaction->warehouse_id = $lims_sale_data->warehouse_id;
                    
                    $transaction->credit = 0;
                    $accountType = ChartofAccount::where('name', 'Cost of Goods Sold')->first();
                    $transaction->chartof_accounts_id = $accountType->id;
                    $transaction->sale_id = $lims_sale_data->id;
                    ...
                    $transaction->user_id = Auth::id();
                    $transaction->warehouse_id = $lims_sale_data->warehouse_id;
                    $transaction->credit = $product_cost;
                    $transaction->debit = 0;
                    ...
                    $transaction->user_id = Auth::id();
                    $transaction->warehouse_id = $lims_sale_data->warehouse_id;
                    $transaction->credit =$lims_sale_data->grand_total-($lims_sale_data->total_tax + $lims_sale_data->order_tax);
                    $transaction->debit = 0;
                    $accountType = ChartofAccount::where('name', 'Sales', '')->first();
                    $transaction->chartof_accounts_id = $accountType->id;
                    $transaction->sale_id = $lims_sale_data->id;
                    ...
                    if ($lims_sale_data->total_tax > 0 || $lims_sale_data->order_tax > 0) {
                        # code...
                        $transaction = new AccountTransaction;
                        $transaction->reference_no = $lims_sale_data->reference_no;
                        $transaction->date = date("Y-m-d H:i:s");
                        $transaction->user_id = Auth::id();
                        $transaction->credit = $lims_sale_data->total_tax + $lims_sale_data->order_tax;
                        $transaction->warehouse_id = $lims_sale_data->warehouse_id;
                        $transaction->debit = 0;
                        $accountType = ChartofAccount::where('name', 'Sale taxes payable', '')->first();
                        $transaction->chartof_accounts_id = $accountType->id;
                        $transaction->save();
                    ...
                    if ($lims_payment_data->change > 0 && $lims_payment_data->change != $data['grand_total']) {
                        # code...
                        $transaction = new AccountTransaction;
                        $transaction->reference_no = $lims_sale_data->reference_no;
                        $transaction->date = date("Y-m-d H:i:s");
                        $transaction->user_id = Auth::id();
                        $transaction->debit = $lims_payment_data->change;
                        $transaction->warehouse_id = $lims_sale_data->warehouse_id;
                        $transaction->credit = 0;
                        $accountType = ChartofAccount::where('name', 'Accounts Receivable', '')->first();
                        $transaction->chartof_accounts_id = $accountType->id;
                        $transaction->save();
                        ...
                        $transaction->date = date("Y-m-d H:i:s");
                        $transaction->user_id = Auth::id();
                        $transaction->debit = $data['paid_amount'];
                        $transaction->warehouse_id = $lims_sale_data->warehouse_id;
                        $transaction->credit = 0;
                    ...
                    } elseif (($lims_payment_data->change > 0) && ($lims_payment_data->change == $data['grand_total'])) {
                        # code...
                        $transaction = new AccountTransaction;
                        $transaction->reference_no = $lims_sale_data->reference_no;
                        $transaction->date = date("Y-m-d H:i:s");
                    ...
                    $transaction->user_id = Auth::id();
                    $transaction->warehouse_id = $lims_sale_data->warehouse_id;
                    $transaction->debit =$lims_sale_data->grand_total;
                    $transaction->credit = 0;
                    $accountType = ChartofAccount::where('name', 'Accounts Receivable', '')->first();
                    $transaction->chartof_accounts_id = $accountType->id;
                    $transaction->save();
                ...
                if ($data['payment_status'] == 3 || $data['payment_status'] == 4 || ($data['payment_status'] == 2 && $data['pos'] && $data['paid_amount'] > 0)) {
                    ...
                    if ($data['paid_amount']) {
                        ...
                        $transaction->debit = 0;
                        $transaction->warehouse_id = $lims_sale_data->warehouse_id;
                        $transaction->credit = $data['paid_amount'];
        ...
        //     $transaction->user_id = Auth::id();
        //     $transaction->warehouse_id = $lims_sale_data->warehouse_id;
        //     $transaction->credit = $data['paying_amount'];
        //     $transaction->debit = 0;
        //     $accountType = ChartofAccount::where('name', 'Sales', '')->first();
        //     $transaction->chartof_accounts_id = $accountType->id;
        //     $transaction->save();
        ...
        //     $transaction->user_id = Auth::id();
        //     $transaction->warehouse_id = $lims_sale_data->warehouse_id;
        //     $transaction->debit = $data['paying_amount'];
        //     $transaction->credit = 0;
        //     $accountType = ChartofAccount::where('name', 'Accounts Receivable', '')->first();
        //     $transaction->chartof_accounts_id = $accountType->id;
        //     $transaction->save();
        ...
        //     if ($lims_sale_data->total_tax > 0) {
        //         # code...
        //         $transaction = new AccountTransaction;
        //         $transaction->reference_no = $lims_sale_data->reference_no;
        //         $transaction->date = date("Y-m-d H:i:s");
        //         $transaction->user_id = Auth::id();
        //         $transaction->credit = $lims_sale_data->total_tax;
        //         $transaction->warehouse_id = $lims_sale_data->warehouse_id;
        //         $transaction->debit = 0;
        //         $accountType = ChartofAccount::where('name', 'Sale taxes payable', '')->first();
        //         $transaction->chartof_accounts_id = $accountType->id;
        ...
        elseif ($data['pos'])
            ...
            return redirect('sales')->with('not_permitted', $errorMessage);
    ...
    public function posSale()
    {
        ...
        if ($role->hasPermissionTo('sales-add')) {
            ...
            $lims_reward_point_setting_data = RewardPointSetting::latest()->first();
            $lims_tax_list = Tax::where('is_active', true)->get();
            $lims_product_list = Product::select('id', 'name', 'code', 'image')->ActiveFeatured()->whereNull('is_variant')->get(); foreach ($lims_product_list as $key => $product) {
    ...
    public function importSale(Request $request)
    {
        ...
        while (!feof($file_handle)) {
            $current_line = fgetcsv($file_handle);
             if ($current_line && $i > 0) {
                 ...
                 if (Str::contains($current_line[0],'-')) {
                    # code...
                     $product_data[$i-1] = Product::join('product_variants', 'products.id', 'product_variants.product_id')
                    ->select('products.*')
                    ->where('product_variants.item_code', $current_line[0])
                    ->first();
                 } else {
                    # code...
                    $product_data[$i-1] = Product::where('code', $current_line[0])->first();
                 }

 
                if ($product_data[$i-1]->is_variant) {
                    # code...
                    $lims_product_data2 = Product::join('product_variants', 'products.id', 'product_variants.product_id')
                ->select('products.*', 'product_variants.item_code', 'product_variants.variant_id', 'product_variants.additional_price')
                ->where('product_variants.item_code', $current_line[0])
                ->first();

                $product_warehouse = Product_Warehouse::where([
                    ['product_id', $product_data[$i-1]->id],
                    ['variant_id',$lims_product_data2->variant_id],
                    ['warehouse_id', $data['warehouse_id']]
                ])->first();

                if(isset($checkQuantity[$current_line[0]])){
                    $checkQuantity[$current_line[0]] += $current_line[1];
                } else {
                    $checkQuantity[$current_line[0]] = $current_line[1];
                }
                if ($checkQuantity[$current_line[0]]>$product_warehouse->qty) {
                    # code...
                    return redirect()->back()->with('message', 'Product '.$current_line[0].' does not enough quantiy!');
        ...
        foreach ($product_data as $key => $product) {
            ...
            if ($data['sale_status'] == 1 && $unit[$key] != 'n/a') {
                ...
                $product->save();
                $child_data = Product::find($product['id'])->first();
                if ($child_data->is_variant) {
                    # code...
                    $lims_product_data = Product::join('product_variants', 'products.id', 'product_variants.product_id')
                ->select('products.*', 'product_variants.item_code', 'product_variants.variant_id', 'product_variants.additional_price')
                ->where('product_variants.item_code', $child_data->product_code)
                ->first();
    ...
    public function edit($id)
    {
        ...
        if ($role->hasPermissionTo('sales-edit')) {
            ...
            if ($lims_sale_data->sale_status==1) {
                # code...
                throw new Exception( ' Sorry! you can not edit Completed sales!');
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($data['grand_total'] == $data['paid_amount']){
            $data['payment_status'] = 4;
        }elseif ($balance ==$data['grand_total']) {
            # code...
        ...
        foreach ($lims_product_sale_data as $key => $product_sale_data) {
            ...
            if (($lims_sale_data->sale_status == 1) && ($lims_product_data->type == 'combo')) {
                ...
                if ($product_sale_data->variant_id) {
                    ...
                    $lims_product_warehouse_data = Product_Warehouse::FindProductWithVariant($product_sale_data->product_id, $product_sale_data->variant_id, $lims_sale_data->warehouse_id,1)
                        ->first();
                    ...
                    $lims_product_warehouse_data = Product_Warehouse::FindProductWithoutVariant($product_sale_data->product_id, $lims_sale_data->warehouse_id,1)
                        ->first();
        ...
        foreach ($product_id as $key => $pro_id) {
            ...
            if ($sale_unit[$key] != 'n/a') {
                ...
                if ($data['sale_status'] == 1) {
                    ...
                    if ($lims_product_data->is_variant) {
                        ...
                        $lims_product_warehouse_data = Product_Warehouse::FindProductWithVariant($pro_id, $lims_product_variant_data->variant_id, $data['warehouse_id'],1)
                            ->first();
                        ...
                        $lims_product_warehouse_data = Product_Warehouse::FindProductWithoutVariant($pro_id, $data['warehouse_id'],1)
                            ->first();
                    }
                    $lims_product_data->qty -= $new_product_qty;
                    if ($lims_product_warehouse_data->qty<=0) {
                        # code...
                        throw new Exception( $lims_product_data->name.' is not Available in selected warehouse please make purchase for this product firist');
            ...
            if ($product_sale['variant_id'] && in_array($product_variant_id[$key], $old_product_variant_id)) {
                ...
                $recent_sale=Product_Sale::where([
                    ['product_id', $pro_id],
                    ['variant_id', $product_sale['variant_id']],
                    ['sale_id', $id]
                ])->first();
        ...
        // $current_product_sale->save();
         ...
         $lims_payment_data = Payment::where('sale_id',$lims_sale_data->id)->first();
        if ($data['sale_status'] == 1) {

            if ($data['payment_status'] == 3 || $data['payment_status'] == 4 || ($data['payment_status'] == 2  && $data['paid_amount'] > 0)) {
                # code...
                if ($lims_payment_data->amount>0) {
                    # code...
                    $transaction = new AccountTransaction;
                        $transaction->reference_no = $lims_sale_data->reference_no;
                        $transaction->date = date("Y-m-d H:i:s");
         ...
         // Commit the transaction if all code executed successfully
         DB::commit();
    ...
    public function addPayment(Request $request)
    {
        ...
        $dataad['is_adjustment'] = false ;
        $lims_AccountTransactionAdjustment_data = AccountTransactionAdjustment::create($dataad);
        if ($lims_payment_data->change > 0 && $lims_payment_data->change != $data['paying_amount']) {
            # code...
            $transaction = new AccountTransaction;
            $transaction->reference_no = $lims_sale_data->reference_no;
            $transaction->date = date("Y-m-d H:i:s");
            $transaction->user_id = Auth::id();
            $transaction->warehouse_id = $lims_sale_data->warehouse_id;
            $transaction->credit = $lims_payment_data->change;
            $transaction->debit = 0;
            $accountType = ChartofAccount::where('name', 'Accounts Receivable', '')->first();
            $transaction->chartof_accounts_id = $accountType->id;
            $transaction->save();
        ...
        return redirect('sales')->with('not_permitted', 'An error occurred while creating the payment.'.$e->getMessage());
    }
}
    ...
    public function updatePayment(Request $request)
    {
        ...
        elseif ($data['edit_paid_by_id'] == 2) {
            ...
            if ($lims_payment_data->paying_method == 'Cheque') {
                ...
                $data['cheque_no'] = $request->input('edit_cheque_no');
                $data['cheque_bank'] = $lims_account_data->name;
        ...
        DB::rollBack();
        $message = 'An error occurred while updating the payment: ' . $e->getMessage();
        return redirect('sales')->with('not_permitted', $message);
...
public function deletePayment(Request $request)
{
    ...
    try {
        ...
        return back()->with('not_permitted', 'An error occurred. Payment deletion failed.');
    }
}
    ...
    public function todaySale()
    {
        ...
        $data['pos_payment'] = Payment::where([
            ['paying_method', 'POS ATM']
        ])->whereDate('created_at', date("Y-m-d"))->sum('amount');
    ...
    public function secondHandList(Request $request)
    {
        ...
        if($role->hasPermissionTo('sales-index')) {
            ...
            if (empty($all_permission))
                ...
                if(Auth::user()->role_id > 2){
                    $lims_secondhandlist_all= ProductPurchase::join('products', 'products.id', '=', 'product_purchases.product_id')
                    ->join('purchases', 'purchases.id', '=', 'product_purchases.purchase_id')
                    ->leftjoin('product_variants', 'product_purchases.variant_id', '=', 'product_variants.variant_id')
                    ->where([
                        ['product_purchases.is_new', 0],
                        ['product_purchases.is_sold', 0],
                        ['product_purchases.warehouse_id',Auth::user()->warehouse_id]
                     ])
                     ->select('product_purchases.*', 'products.name','purchases.reference_no','product_variants.item_code')
                    ...
                    $lims_secondhandlist_all= ProductPurchase::join('products', 'products.id', '=', 'product_purchases.product_id')
                    ->join('purchases', 'purchases.id', '=', 'product_purchases.purchase_id')
                    ->leftjoin('product_variants', 'product_variants.variant_id', '=', 'product_purchases.variant_id')
                    ->where([
                        ['product_purchases.is_new', 0],
                        ['product_purchases.is_sold', 0],
                        ['product_purchases.warehouse_id',$warehouse_id]
                     ])
                     ->select('product_purchases.*', 'products.name','purchases.reference_no','product_variants.item_code')
                     ->orderBy('created_at', 'desc')
                    ->get();
         ...
         $lims_warehouse_list =  Warehouse::where([
            ...
            $lims_account_list = Account::where('is_active', true)->get();
            return view('sale.secondhandlist', compact('lims_secondhandlist_all', 'all_permission', 'lims_account_list'));
        }
        else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
    ...
    public function createsecondhandsale(Request $request)
    {
        ...
        if ($role->hasPermissionTo('sales-index')) {
            ...
            if ($lims_purchase_data) {
                ...
                $lims_product_purchase_data = ProductPurchase::where('id',$request->input('product_purchase'))->get();
            }else {
                # code...
                return redirect('secondhand/sale')->with('not_permitted', 'Sorry! we can not find your data');
            ...
            $lims_pos_setting_data = PosSetting::latest()->first();
            $lims_reward_point_setting_data = RewardPointSetting::latest()->first(); 
            return view('sale.createsecondhandsale', compact('lims_reward_point_setting_data','lims_pos_setting_data','lims_biller_list','lims_customer_list','lims_warehouse_list', 'lims_tax_list', 'lims_account_list', 'lims_purchase_data', 'lims_product_purchase_data','warehouse_id'));
        } else
        return redirect('secondhand/sale')->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
    ...
    public function store2(Request $request)
    {
        ...
        if ($data['pos']) {
            ...
            if (!isset($data['reference_no'])){
                $data['reference_no'] = 'sr-secondhand-' . date("Ymd") . '-' . date("his");
            }else{
                $data['reference_no'] = 'sr-secondhand-' . date("Ymd") . '-' . date("his");
...
// dd($data);
        ...
        $tax = $data['tax'];
        $total = $data['subtotal'];
        $product_purchase_id = $data['product_purchase_id'];
        $product_sale = [];
        $currentSale = collect();
        ...
        foreach ($product_id as $i => $id) {
            ...
            if ($sale_unit[$i] != 'n/a') {
                ...
                if ($data['sale_status'] == 1) {
                    ...
                    if ($lims_product_data->is_variant) {
                        $lims_product_variant_data->qty -= $quantity;
                        $lims_product_variant_data->save();
                        $lims_product_warehouse_data = Product_Warehouse::FindProductWithVariant($id, $lims_product_variant_data->variant_id, $data['warehouse_id'], 0)->first();
                    } elseif ($product_batch_id[$i]) {
                        $lims_product_warehouse_data = Product_Warehouse::where([
                            ['product_batch_id', $product_batch_id[$i]],
                            ['warehouse_id', $data['warehouse_id']],
                        ...
                        $lims_product_warehouse_data = Product_Warehouse::FindProductWithoutVariant($id, $data['warehouse_id'], 0)->first();
                    }
                    //deduct quantity from warehouse
                    $lims_product_warehouse_data->qty -= $quantity;
                    $lims_product_warehouse_data->save();
            ...
            $product_sale['tax_rate'] = $tax_rate[$i];
            $product_sale['tax'] = $tax[$i];
            $product_sale['total'] = $mail_data['total'][$i] = $qty[$i]* $net_unit_price[$i];
            $product_sale['warehouse_id'] =$lims_sale_data->warehouse_id;
            $recent_sale = Product_Sale::create($product_sale);
            $product_cost=$net_unit_cost[$i];
            $recent_sale = Product_Sale::latest()->first();
            $product_sale_purchase_cost= new product_sale_purchase_cost();


            $product_sale_purchase_cost->product_purchase_id=$product_purchase_id[$i];
            $product_sale_purchase_cost->product_sale_id=$recent_sale->id;
            $product_sale_purchase_cost->qty=$qty[$i];
            $product_sale_purchase_cost->save();

            $sold_purchase = ProductPurchase::where('id',$product_purchase_id[$i])->first();
            if($sold_purchase->qty ==$qty[$i]){
                $sold_purchase->sold_qty=$qty[$i];
                $sold_purchase->is_sold=1;
                $sold_purchase->save();
            }{
                $sold_purchase->sold_qty=$qty[$i];
       ...
       $journal_entry['lims_AccountTransactionAdjustment_id'] =$lims_AccountTransactionAdjustment_data->id;
        ...
        elseif ($data['pos'])
            ...
            return redirect('sales')->with('not_permitted', $errorMessage.$e->getLine());
