    ...
    public function create(Request $request)
    {
        ...
        if($role->hasPermissionTo('returns-add')) {
            ...
            if ($lims_sale_data) {
                ...
                # code...
                return redirect('return-sale')->with('not_permitted', 'Sorry! we can not find your data');
    ...
    public function store(Request $request)
    {
        ...
        $data = $request->except('document');
         ...
         if (!isset($data['is_return'])) {
            # code...
           return  redirect('return-sale')->with("not_permitted","There is no selected sale record for return");
        }
        $data['reference_no'] = 'sales-return-r-' . date("Ymd") . '-'. date("his");
        $data['user_id'] = Auth::id();
        $lims_sale_data = Sale::find($data['sale_id']);
        $lims_sale_data->returned_qty=$lims_sale_data->returned_qty + $data['total_qty'];
        $lims_sale_data->save();
        $data['user_id'] = Auth::id();
        ...
        $transaction->reference_no =  $lims_return_data->reference_no;
        $transaction->date = date("Y-m-d H:i:s");
        $transaction->user_id	 = Auth::id();
        $transaction->warehouse_id =  $data['warehouse_id']; 
        $transaction->credit =array_sum($data['subtotal']);
        $transaction->debit =  0;
        $accountType = ChartofAccount::where('name', $lims_account_data->cname)->first();
        ...
        foreach ($product_code_selected as $pcs) {
            ...
            if($sale_unit[$key] != 'n/a') {
                ...
                if($lims_product_data->is_variant) {
                    ...
                    $update_warehousesales_data = Product_Sale::where([
                        ['product_id', $pro_id],
                        ['variant_id', $lims_product_variant_data->variant_id],
                        ['sale_id', $data['sale_id']]
                    ])->first();
                    $update_warehousesales_data->returned_qty=$update_warehousesales_data->returned_qty +  $quantity;
                    $update_warehousesales_data->save();
                }
                ...
                $product_sale_purchase_cost = product_sale_purchase_cost::where([
                     ['product_sale_id', $product_sale_id[$key]]
                ])->get();

                if ($product_sale_purchase_cost) {
                    # code...
                    $tempoquantity=$quantity;
                foreach ($product_sale_purchase_cost as  $pspc) {
                    # code...
                    if ($tempoquantity>0) {
                        # code...
                        if ($pspc->qty >= $tempoquantity) {
                            # code...
                            $product_purchase = ProductPurchase::where([
                                ['id', $pspc->product_purchase_id]
                           ])->first();
    
                           $cost_of_good_sold+=$tempoquantity*$product_purchase->net_unit_cost;
                            $pspc->qty-=$tempoquantity;
                            $pspc->save();
                            $tempoquantity=0;

    
                            
                            $product_purchase->sold_qty-=$tempoquantity;
                            $product_purchase->is_sold=0;
                           ...
                           $cost_of_good_sold+=$pspc->qty*$product_purchase->net_unit_cost;
                            $pspc->qty=0;
                            $pspc->save();
                            $tempoquantity-=$pspc->qty;
    
                            $product_purchase->sold_qty-=$pspc->qty;
                            $product_purchase->is_sold=0;
                            $product_purchase->save();
        ...
        $transaction->warehouse_id =  $data['warehouse_id']; 
        $transaction->debit = 0;
        $transaction->credit = $cost_of_good_sold;
        ...
        $transaction->warehouse_id =  $data['warehouse_id']; 
        $transaction->credit =0;
        $transaction->debit = $cost_of_good_sold;
        ...
        return redirect('return-sale')->with('not_permitted', $errorMessage);
