...
<html dir="@if( Config::get('app.locale') == 'ar' || $general_setting->is_rtl){{'rtl'}}@endif">
  ...
  if ('serviceWorker' in navigator) {
    window.addEventListener('load', function() {
      navigator.serviceWorker.register('{{ asset('service-worker.js', true) }}')
...
<script>
    ...
    <div id="loader"></div>
      ...
      <nav class="side-navbar">
        ...
        <ul id="side-main-menu" class="side-menu list-unstyled">
            ...
            <li><a href="#activity_log" aria-expanded="false" data-toggle="collapse"> <i class="dripicons-clock"></i><span>Log</span></a>
            <ul id="activity_log" class="collapse list-unstyled ">
                @if($sale_return_index_permission_active)
                <li id="activity_log-list-menu"><a href="{{route('activity_log.index')}}">Active Logs</a></li>
                @endif
                @if($purchase_return_index_permission_active)
                <li id="activity_log_history-list-menu"><a href="{{route('log_history')}}">Log History</a></li>
                @endif
            </ul>
            </li>
            @endif
            ...
            <?php
                $index_permission_active = DB::table('permissions')
                    ->join('role_has_permissions', 'permissions.id', '=', 'role_has_permissions.permission_id')
                    ->where([
                        ['permissions.name', 'warehousepurchases-index'],
                        ['role_id', $role->id] 
                    ])->first();
            ?>
            @if($index_permission_active)
            <li><a href="#purchasewarehouse" aria-expanded="false" data-toggle="collapse"> <i class="dripicons-card"></i><span>Purchase in Warehouse</span></a>
            <ul id="purchasewarehouse" class="collapse list-unstyled ">
                <li id="purchasewarehouse-list-menu"><a href="{{route('warehousepurchases.index')}}">Purchase List in Warehouse </a></li>
                <?php
                    $add_permission_active = DB::table('permissions')
                        ->join('role_has_permissions', 'permissions.id', '=', 'role_has_permissions.permission_id')
                        ->where([
                            ['permissions.name', 'warehousepurchases-add'],
                            ['role_id', $role->id] 
                        ])->first();
                ?>
                @if($add_permission_active)
                <li id="purchasewarehouse-create-menu"><a href="{{route('warehousepurchases.create')}}">Add Purchase to Warehouse</a></li>
                <li id="purchasewarehouse-import-menu"><a href="{{url('warehousepurchases/purchase_by_csv')}}">{{trans('file.Import Purchase By CSV')}}</a></li>
                @endif
            </ul>
            </li>
            @endif
            ...
            <?php
                ...
                $purchase_returntransfer_index_permission_active = DB::table('permissions')
                    ->join('role_has_permissions', 'permissions.id', '=', 'role_has_permissions.permission_id')
                    ->where([
                        ['permissions.name', 'purchase-return-index'],
                        ['role_id', $role->id] 
            ...
            ?>
            @if($purchase_returntransfer_index_permission_active)
            <li><a href="#PurchaseTransfer" aria-expanded="false" data-toggle="collapse"> <i class="dripicons-export"></i><span>Purchase Transfer</span></a>
            <ul id="PurchaseTransfer" class="collapse list-unstyled ">
                 
                @if($purchase_returntransfer_index_permission_active)
                <li id="purchase-transfer-menu"><a href="{{route('transfer-purchase.index')}}">{{trans('file.Purchase')}}</a></li>
                @endif
            </ul>
            </li>
            @endif
            ...
            <ul id="sale" class="collapse list-unstyled ">
                ...
                <li id="sale-secondhand-menu"><a href="{{route('sales.secondhand')}}">Sale SecondHand(used)</a></li>
            ...
            <?php
                $index_permission_active = DB::table('permissions')
                    ->join('role_has_permissions', 'permissions.id', '=', 'role_has_permissions.permission_id')
                    ->where([
                        ['permissions.name', 'transaction_adjustments-index'],
                        ['role_id', $role->id] 
                    ])->first();
            ?>
            @if($index_permission_active)
            <li><a href="#transaction_adjustments" aria-expanded="false" data-toggle="collapse"> <i class="dripicons-wallet"></i><span>Transaction Adjustment</span></a>
            <ul id="transaction_adjustments" class="collapse list-unstyled ">
                <li id="transaction_adjustments-cat-menu"><a href="{{route('chart_of_accounts.index')}}">Chart Of Accounts</a></li>
                <li id="transaction_adjustments-list-menu"><a href="{{route('transaction_adjustments.index')}}">Transaction Adjustment List</a></li>
                @if($add_permission_active)
                <li id="transaction_adjustments-create-menu"><a href="{{route('transaction_adjustments.create')}}">Add Transaction Adjustment</a></li>
                ...
                <?php
                    $add_permission_active = DB::table('permissions')
                        ->join('role_has_permissions', 'permissions.id', '=', 'role_has_permissions.permission_id')
                        ->where([
                            ['permissions.name', 'transaction_adjustments-add'],
                            ['role_id', $role->id] 
                        ])->first();
                ?>
            ...
            <?php 
                $index_permission = DB::table('permissions')->where('name', 'buses-index')->first();
              ...
              ?>
              @if($index_permission_active)
              <li><a href="#bus" aria-expanded="false" data-toggle="collapse"> <i class="dripicons-wallet"></i><span>Buses</span></a>
                <ul id="bus" class="collapse list-unstyled ">
                  <li id="bus-list-menu"><a href="{{route('buses.index')}}">Buses List</a></li>
                  <?php 
                    $add_permission = DB::table('permissions')->where('name', 'buses-add')->first();
                  ...
                  ?>
                  @if($add_permission_active)
                  <li><a id="add-buses" href=""> Add Bus</a></li>
                  @endif
 
                  @if($add_permission_active)
                  <li><a id="add-cities" href=""> Add City</a></li>
                  @endif

                  <li id="bus-list-menu"><a href="{{route('cities.index')}}">City List</a></li>

                </ul>
              </li>
              @endif
              <?php 
                $index_permission = DB::table('permissions')->where('name', 'ticket-index')->first();
                $index_permission_active = DB::table('role_has_permissions')->where([
                        ['permission_id', $index_permission->id],
                        ['role_id', $role->id]
                    ])->first();
           ...
           @if($index_permission_active)
              <li id="ticket"><a href="{{route('ticket.create')}}" aria-expanded="false" > <i class="dripicons-document"></i><span>Ticketing</span></a>

             @endif

              <?php 
                $index_permission = DB::table('permissions')->where('name', 'route-index')->first();
                $index_permission_active = DB::table('role_has_permissions')->where([
                        ['permission_id', $index_permission->id],
                        ['role_id', $role->id]
                    ])->first();
             ...
             @if($index_permission_active)
              <li><a href="#route" aria-expanded="false" data-toggle="collapse"> <i class="dripicons-wallet"></i><span>Routes</span></a>
                <ul id="route" class="collapse list-unstyled ">
                  <li id="route-list-menu"><a href="{{route('routes1.index')}}">Routes List</a></li>
                  <?php 
                    $add_permission = DB::table('permissions')->where('name', 'route-add')->first();
                    $add_permission_active = DB::table('role_has_permissions')->where([
                        ['permission_id', $add_permission->id],
                        ['role_id', $role->id]
                    ])->first();
                  ?>
                  @if($add_permission_active)
                  <li><a id="add-route" href=""> Add Route</a></li>
                  <li id="bus-list-menu"><a href="{{route('queue.index')}}">Buses  QUE</a></li>
                  @endif
                </ul>
              </li>
              @endif
              ...
              <?php
                $index_permission_active = DB::table('permissions')
                    ->join('role_has_permissions', 'permissions.id', '=', 'role_has_permissions.permission_id')
                    ->where([
                        ['permissions.name', 'fixed_asset-index'],
                        ['role_id', $role->id] 
                    ])->first();
            ?>
            @if($index_permission_active)
            <li><a href="#fixed_asset" aria-expanded="false" data-toggle="collapse"> <i class="dripicons-wallet"></i><span>Fixed Asset</span></a>
            <ul id="fixed_asset" class="collapse list-unstyled ">
                <li id="fixed_asset-cat-menu"><a href="{{route('fixed_asset_categories.index')}}">Fixed Asset Category</a></li>
                <li id="fixed_asset-list-menu"><a href="{{route('fixed_asset.index')}}">Fixed Asset List</a></li>
                <li id="fixed_asset-list-menu"><a href="{{route('fixed_asset.index')}}">Prepaid Rents</a></li>
                <?php
                    $add_permission_active = DB::table('permissions')
                        ->join('role_has_permissions', 'permissions.id', '=', 'role_has_permissions.permission_id')
                        ->where([
                            ['permissions.name', 'fixed_asset-add'],
                            ['role_id', $role->id] 
                        ])->first();
                ?>
                @if($add_permission_active)
                <li><a id="add-fixed_asset" href=""> Add Fixed Asset</a></li>
                <li><a id="add-prepaid_rent" href=""> Add Prepaid Rent</a></li>
                @endif
            </ul>
            </li>
            @endif
            ...
            <ul id="account" class="collapse list-unstyled ">
                @if($index_permission_active)
                <li id="account-list-menu"><a href="{{route('accounts.index')}}">Bank {{trans('file.Account List')}}</a></li>
                <li><a id="add-account" href="">{{trans('file.Add Bank Account')}}</a></li>
                
                 <li id="journal-entry-menu"><a href="{{route('journal_entries')}}">Journal Entries</a></li>

                     
                  
                  
                <li id="general-ledger-menu">
                    {!! Form::open(['route' => 'accounttransaction.ledger', 'method' => 'GET', 'id' => 'ledger-report-form']) !!}
                    <input type="hidden" name="start_date" value="{{$general_setting->fiscal_year}}" />
                    <input type="hidden" name="end_date" value="{{date('Y-m-d')}}" />
                    <input type="hidden" name="warehouse_id" value="0" />
                    <a id="ledger-report-link" href="">General Ledger</a>
                    {!! Form::close() !!}
                  </li>
                  <li id="financial-menu">
                    {!! Form::open(['route' => 'accounttransaction.financialstatement', 'method' => 'get', 'id' => 'financialstatement-report-form']) !!}
                    <input type="hidden" name="start_date" value="{{date('Y-m').'-'.'01'}}" />
                    <input type="hidden" name="end_date" value="{{date('Y-m-d')}}" />
                     <a id="financialstatement-report-link" href="">Financial Statements</a>
                    {!! Form::close() !!}
                  </li>
                  <li id="financial-Trial_Balance-menu">
                    {!! Form::open(['route' => 'accounttransaction.Trial_Balance', 'method' => 'get', 'id' => 'Trial_Balance-report-form']) !!}
                    <input type="hidden" name="start_date" value="{{date('Y-m').'-'.'01'}}" />
                    <input type="hidden" name="end_date" value="{{date('Y-m-d')}}" />
                    <input type="hidden" name="warehouse_id" value="0" />
                    <a id="Trial_Balance-report-link" href="">Trial Balance</a>
                    {!! Form::close() !!}
                  </li>


                  <li id="financial-Income_Statement-menu">
                    {!! Form::open(['route' => 'accounttransaction.Income_Statement', 'method' => 'get', 'id' => 'IncomeStatement-report-form']) !!}
                    <input type="hidden" name="start_date" value="{{date('Y-m').'-'.'01'}}" />
                    <input type="hidden" name="end_date" value="{{date('Y-m-d')}}" />
                    <input type="hidden" name="warehouse_id" value="0" />
                    <a id="IncomeStatement-report-link" href="">Income Statements</a>
                    {!! Form::close() !!}
                  </li>

                  <li id="financial-Balance_Sheet-menu">
                    {!! Form::open(['route' => 'accounttransaction.Balance_Sheet', 'method' => 'get', 'id' => 'Balance_Sheet-report-form']) !!}
                    <input type="hidden" name="start_date" value="{{date('Y-m').'-'.'01'}}" />
                    <input type="hidden" name="end_date" value="{{date('Y-m-d')}}" />
                     <a id="Balance_Sheet-report-link" href="">Balance Sheet</a>
                    {!! Form::close() !!}
                  </li>
                @endif
                ...
                <li id="balance-sheet-menu"><a href="{{route('accounts.balancesheet')}}">Bank {{trans('file.Balance Sheet')}}</a></li>
                @endif
                @if($account_statement_permission_active)
                <li id="account-statement-menu"><a id="account-statement" href="">Bank {{trans('file.Account Statement')}}</a></li>
                @endif
            </ul>
            </li>
            @endif
            ...
            <?php
                $index_permission_active = DB::table('permissions')
                    ->join('role_has_permissions', 'permissions.id', '=', 'role_has_permissions.permission_id')
                    ->where([
                        ['permissions.name', 'shareholders-index'],
                        ['role_id', $role->id] 
                    ])->first();

                $shareholders_add = DB::table('permissions')
                    ->join('role_has_permissions', 'permissions.id', '=', 'role_has_permissions.permission_id')
                    ->where([
                        ['permissions.name', 'shareholders-add'],
                        ['role_id', $role->id] 
                    ])->first();

                $shareholders_edit = DB::table('permissions')
                    ->join('role_has_permissions', 'permissions.id', '=', 'role_has_permissions.permission_id')
                    ->where([
                        ['permissions.name', 'shareholders-edit'],
                        ['role_id', $role->id] 
                    ])->first();

                $shareholders_delete = DB::table('permissions')
                    ->join('role_has_permissions', 'permissions.id', '=', 'role_has_permissions.permission_id')
                    ->where([
                        ['permissions.name', 'shareholders-delete'],
                        ['role_id', $role->id] 
                    ])->first();
            ...
            @if($index_permission_active ||  $shareholders_add || $shareholders_edit)
            <li class=""><a href="#share" aria-expanded="false" data-toggle="collapse"> <i class="dripicons-briefcase"></i><span>Shares</span></a>
            <ul id="share" class="collapse list-unstyled ">
                @if($index_permission_active)
                 <li id="shareholder-menu"><a href="{{route('shareholders.index')}}">ShareHolders</a></li>
            ...
            <ul id="hrm" class="collapse list-unstyled ">
                ...
                @endif
                @if($payroll_active)
                <li id="create_payroll-menu">
                    {!! Form::open(['route' => 'payroll.create_payroll', 'method' => 'get', 'id' => 'create_payroll']) !!}
                    <input type="hidden" name="warehouse_id" value="0" />
                    <a id="create_payroll-link" href="">Create Payroll</a>
                    {!! Form::close() !!}
                  </li>
                <li id="payroll-menu"><a href="{{route('payroll.indexone')}}">{{trans('file.Payroll')}}</a></li>
                <li id="perdime-menu"><a href="{{route('perdime.index')}}">Perdime</a></li>
                @endif
    ...
    <div class="page">
      ...
      <header class="container-fluid">
        <nav class="navbar">
            ...
            @if($product_qty_alert_active && ( count(\Auth::user()->unreadNotifications) + $lims_activity_log_list_count) > 0)
                <li class="nav-item" id="notification-icon">
                    <a rel="nofollow" data-toggle="tooltip" title="{{__('Notifications')}}" class="nav-link dropdown-item"><i class="dripicons-bell"></i><span class="badge badge-danger notification-number">{{$lims_activity_log_list_count + count(\Auth::user()->unreadNotifications)}}</span>
                    </a>
                    <ul class="right-sidebar">
                        

                        @if($lims_activity_log_list_count>0)
                        <li class="notifications">
                            <a href="{{route('activity_log.index')}}" class="btn btn-link"> {{$lims_activity_log_list_count}} Logs  needs Urgent Approval or Rejection </a>
                        </li>
                        @endif
            ...
            <li class="nav-item">
                    ...
                    <ul class="right-sidebar">
                        ...
                        <li>
                        <a href="{{ url('language_switch/am') }}" class="btn btn-link"> Amharic</a>
                        </li>
                        <li>
      ...
      <!-- bus modal -->
      <div id="bus-modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true" class="modal fade text-left">
        <div role="document" class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 id="exampleModalLabel" class="modal-title">Add bus</h5>
                    <button type="button" data-dismiss="modal" aria-label="Close" class="close"><span aria-hidden="true"><i class="dripicons-cross"></i></span></button>
                </div>
                <div class="modal-body">
                  <p class="italic"><small>{{trans('file.The field labels marked with * are required input fields')}}.</small></p>
                    {!! Form::open(['route' => 'buses.store', 'method' => 'post']) !!}
 ...
 <!-- route modal -->
 <div id="route-modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true" class="modal fade text-left">
        <div role="document" class="modal-dialog">
        ...
        <?php       
            ...
            <div class="modal-content">
                <div class="modal-header">
                    <h5 id="exampleModalLabel" class="modal-title">Add Route</h5>
                    <button type="button" data-dismiss="modal" aria-label="Close" class="close"><span aria-hidden="true"><i class="dripicons-cross"></i></span></button>
                </div>
                <div class="modal-body">
                  <p class="italic"><small>{{trans('file.The field labels marked with * are required input fields')}}.</small></p>
                    ...
                    <?php
                      ...
                      <div class="col-md-6 form-group">
                        ...
                        <div class="col-md-6 form-group">
                            ...
                            <input type="text" name="TicketPrice" class="form-control" />ETB
                        </div>
      ...
      <!-- city modal -->
      <div id="city-modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true" class="modal fade text-left">
        <div role="document" class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 id="exampleModalLabel" class="modal-title">Add city</h5>
                    <button type="button" data-dismiss="modal" aria-label="Close" class="close"><span aria-hidden="true"><i class="dripicons-cross"></i></span></button>
                </div>
                <div class="modal-body">
                  <p class="italic"><small>{{trans('file.The field labels marked with * are required input fields')}}.</small></p>
      ...
      <!-- fixed_asset modal -->
      <div id="fixed_asset-modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true" class="modal fade text-left">
        <div role="document" class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 id="exampleModalLabel" class="modal-title">Add Fixed Asset</h5>
                    <button type="button" data-dismiss="modal" aria-label="Close" class="close"><span aria-hidden="true"><i class="dripicons-cross"></i></span></button>
                </div>
                <div class="modal-body">
                  <p class="italic"><small>{{trans('file.The field labels marked with * are required input fields')}}.</small></p>
                      ...
                      <div class="row">
                        ...
                        <div class="col-md-6 form-group">
                            ...
                            <select name="fixed_asset_category_id" class="selectpicker form-control" required data-live-search="true" data-live-search-style="begins" title="Select Expense Category...">
                                @foreach($lims_fixed_asset_category_list as $fixed_asset_category)
      ...
      <!-- end fixed_asset modal -->


   <!-- fixed_asset modal -->
   <div id="prepaid_rent-modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true" class="modal fade text-left">
        <div role="document" class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 id="exampleModalLabel" class="modal-title">Add PrePaid Rent</h5>
                    <button type="button" data-dismiss="modal" aria-label="Close" class="close"><span aria-hidden="true"><i class="dripicons-cross"></i></span></button>
                </div>
                <div class="modal-body">
                  <p class="italic"><small>{{trans('file.The field labels marked with * are required input fields')}}.</small></p>
      ...
      <!-- end fixed_asset modal -->
            ...
            <!-- transaction_adjustments modal -->
            <div id="transaction_adjustments-modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true" class="modal fade text-left">
        <div role="document" class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 id="exampleModalLabel" class="modal-title">Add Transaction Adjustment</h5>
                    <button type="button" data-dismiss="modal" aria-label="Close" class="close"><span aria-hidden="true"><i class="dripicons-cross"></i></span></button>
                </div>
                <div class="modal-body">
                  <p class="italic"><small>{{trans('file.The field labels marked with * are required input fields')}}.</small></p>
                    ...
                    ?>
                      <div class="row">
                        ...
                        <div class="col-md-6 form-group">
                            ...
                            <select name="chartof_accounts_id" class="selectpicker form-control" required data-live-search="true" data-live-search-style="begins" title="Select chart of Account...">
                                @foreach($lims_expense_category_list as $expense_category)
                        ...
                        <div class="col-md-6 form-group">
                            <label>Credit *</label>
                            <input type="number" name="credit" step="any" required class="form-control" min=0  value="0">
                        </div>
                        <div class="col-md-6 form-group">
                            <label>Debit *</label>
                            <input type="number" name="debit" step="any" required class="form-control" min=0 value="0">
                        </div>
      ...
      <!-- end transaction_adjustment modal -->
                ...
                <div class="modal-body">
                  <p class="italic"><small>{{trans('file.The field labels marked with * are required input fields')}}.</small></p>
                    {!! Form::open(['route' => 'accounts.store', 'method' => 'post']) !!}
                      ...
                      <div class="form-group">
                          <label>Bank {{trans('file.name')}} *</label>
                      ...
                      <div class="form-group">
                          ...
                          <input type="number" name="initial_balance" value="0" step="any" class="form-control">
                      </div>
                      <div class="form-group">
                          <label>{{trans('file.Note')}}</label>
                          <textarea name="note" rows="3" class="form-control"></textarea>
      ...
      <footer class="main-footer">
        <div class="container-fluid">
          <div class="row">
            <div class="col-sm-12">
              <p>&copy; {{$general_setting->site_title}} | {{trans('file.Developed')}} {{trans('file.By')}} <span class="external">{{$general_setting->developed_by}}</span> | 0919176988|0970493518</p>
            </div>
          </div>
        </div>
      </footer>
    ...
    <script>
        if ('serviceWorker' in navigator ) {
            window.addEventListener('load', function() {
                navigator.serviceWorker.register('/Invosale2/service-worker.js').then(function(registration) {
    ...
    <script type="text/javascript">
      ...
      $("div.alert").delay(20000).slideUp(1000);
...
if(userid<2){
    alert_c=alert_product+lims_activity_log_list_count;
}
      $("li#notification-icon").on("click", function (argument) {
          $.get('notifications/mark-as-read', function(data) {
              $("span.notification-number").text(alert_c);
          });
      });  
      ...
      $("a#add-buses").click(function(e){
        e.preventDefault();
        $('#bus-modal').modal();
      });

      $("a#add-route").click(function(e){
        e.preventDefault();
        $('#route-modal').modal();
      });
      $("a#add-cities").click(function(e){
        e.preventDefault();
        $('#city-modal').modal();
      });
      $("a#add-fixed_asset").click(function(e){
        e.preventDefault();
        $('#fixed_asset-modal').modal();
      });

      $("a#add-prepaid_rent").click(function(e){
        e.preventDefault();
        $('#prepaid_rent-modal').modal();
      });


      $("a#add-transaction_adjustments").click(function(e){
        e.preventDefault();
        $('#transaction_adjustments-modal').modal();
      });
      ...
      $("a#create_payroll-link").click(function(e){
        e.preventDefault();
        $("#create_payroll").submit();
      });
      ...
      $("a#ledger-report-link").click(function(e){
        e.preventDefault();
        $("#ledger-report-form").submit();
      });
      $("a#financialstatement-report-link").click(function(e){
        e.preventDefault();
        $("#financialstatement-report-form").submit();
      });

      $("a#IncomeStatement-report-link").click(function(e){
        e.preventDefault();
        $("#IncomeStatement-report-form").submit();
      });

      $("a#Balance_Sheet-report-link").click(function(e){
        e.preventDefault();
        $("#Balance_Sheet-report-form").submit();
      }); 
      
      $("a#Trial_Balance-report-link").click(function(e){
        e.preventDefault();
        $("#Trial_Balance-report-form").submit();
      });

      $("a#journal-report-link").click(function(e){
        e.preventDefault();
        $("#journal-report-form").submit();
      });
